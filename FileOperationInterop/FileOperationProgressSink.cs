...
namespace FileOperationInterop {
    public class FileOperationProgressSink : IFileOperationProgressSink {
        ...
        public virtual void PostDeleteItem (uint dwFlags, IShellItem psiItem, uint hrDelete, IShellItem psiNewlyCreated) {
        ...
        public virtual void PostNewItem (
            uint dwFlags, IShellItem psiDestinationFolder, string pszNewName,
            string pszTemplateName, uint dwFileAttributes, uint hrNew, IShellItem psiNewItem
        ) {
        ...
        private static void TraceAction (string action, string item, uint hresult) {
            string message = $"{action} ({(CopyEngineResult) hresult})";
